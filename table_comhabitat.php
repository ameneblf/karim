    ...
    <script>
        $(document).ready(function() {
            ...
            $("#valid").click(function() {
                var motif = $("#motifval").val();
                var date1 = $("#date").val();
                var date = $('a[data-role=update]').data('date');
                var reg = $('a[data-role=update]').data('id');
                var id = $('a[data-role=update]').data('com');
                var numcertif = $('#N_Certificat').val();
                console.log(id);
                if (date <= date1) {
                    if ($('input:checked').length == 4) {
                        $.get(
                            ...
                            function(data) {
                                //console.log(data);
                                $('#modalup').modal('hide');
                                var com = $('a[data-role=update]').data('com');
                                var td = $('#' + com).closest('tr').find('td:eq(1)');
                                $('#' + com).closest('tr').find('td:eq(0)').html("" + date1);
                                td.html("<td><span class=\"badge bg-label-success me-1\">valide</span></td>");
                            }
                        );
                    } else {
                        $.get(
                            ...
                            function(data) {
                                ...
                                var com = $('a[data-role=update]').data('com');
                                var td = $('#' + com).closest('tr').find('td:eq(1)');
                                td.html("<td><span class=\"badge bg-label-danger me-1\">refuse</span></td>");
                            }
                        );
                    }
                }
                if (date > date1) {
                    ...
                    $("#alert").html("Veuillez vérifier la date !!");
                    setTimeout(function() {
